...
const Chat = () => {
    ...
    // cbtx is a Q&A-only bot, so the welcome is a plain greeting with no Tutor/Q&A mode buttons
    // (unlike lemon, which appends a [[CHOICES kind=mode]] marker here). The synthetic welcome
    // pair is never sent to the backend.
    ...
    const getConfig = async () => {
        configApi().then(config => {
            const effectiveConfig = applyChatbotSpeechFeatureFlags("cbtx", config);
            ...
            // cbtx keeps agentic retrieval OFF by default (registry agenticRetrievalDefault=false);
            // the toggle is still shown when the deployment enables it, just not auto-checked.
