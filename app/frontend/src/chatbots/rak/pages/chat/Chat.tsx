...
const Chat = () => {
    ...
    const chatbotCategory = "rak";
    const legacyInitialUserMessage: string = t("initialUserMsg");
    const initialAssistantMessageContent: string = t(
        currentUser?.username === "12345"
            ? "initialAssistantMessages.user12345"
            : currentUser?.username === "67890"
              ? "initialAssistantMessages.user67890"
              : "initialAssistantMsg"
    );
    const initialAssistantResponse: ChatAppResponse = useMemo(
        () => ({
            message: {
    ...
    const initialAssistantPair: [user: string, response: ChatAppResponse] = useMemo(
        () => [INITIAL_ASSISTANT_SENTINEL_USER_MESSAGE, initialAssistantResponse],
    ...
    const getConfig = async () => {
        configApi().then(config => {
            const effectiveConfig = applyChatbotSpeechFeatureFlags("rak", config);
    ...
    const makeApiRequest = async (question: string) => {
        ...
        try {
            ...
            const requestContext = {
                ...
                ...(currentUser ? { chatbotUser: currentUser.username } : {})
            };

            const request: ChatAppRequest = {
                messages: [...messages, { content: question, role: "user" }],
                context: requestContext as ChatAppRequest["context"],
    ...
    // Keep the pristine welcome state aligned with the active locale and logged-in RAK user.
    useEffect(() => {
        const hasOnlyPristineWelcome =
            answers.length === 1 &&
            (answers[0][0] === INITIAL_ASSISTANT_SENTINEL_USER_MESSAGE || answers[0][0] === legacyInitialUserMessage);

        if (hasOnlyPristineWelcome && !isLoading && !isStreaming) {
            setAnswers([initialAssistantPair]);
            setStreamedAnswers([initialAssistantPair]);
            setSpeechUrls([null]);
        }
    }, [answers, initialAssistantPair, isLoading, isStreaming, legacyInitialUserMessage]);
    ...
    const shouldOpenCitationDirectly = (citation: string) => {
        const citationWithoutHash = citation.split("#")[0].toLowerCase();
        return (
            citationWithoutHash.endsWith(".pdf") ||
            citationWithoutHash.includes(".pdf?") ||
            citationWithoutHash.endsWith(".csv") ||
            citationWithoutHash.includes(".csv?")
        );
    };

    const onShowCitation = (citation: string, index: number) => {
        if (shouldOpenCitationDirectly(citation)) {
