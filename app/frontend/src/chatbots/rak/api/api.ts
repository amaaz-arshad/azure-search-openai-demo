...
export function getCitationFilePath(
    ...
    const chatbotPathPrefix = options?.chatbotName ? `${options.chatbotName}/` : "";
    return `${BACKEND_URI}/content/${chatbotPathPrefix}${pathWithoutFragment}${query ? `?${query}` : ""}${fragmentSuffix}`;
}
...
export async function listChatbotUploadedFilesApi(chatbotName: string, options?: ChatbotUserOptions): Promise<string[]> {
    const headers: Record<string, string> = {};
    if (options?.chatbotUser) {
        headers["X-Chatbot-User"] = options.chatbotUser;
    }
