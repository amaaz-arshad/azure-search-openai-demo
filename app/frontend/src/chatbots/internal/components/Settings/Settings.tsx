...
import { HelpCallout } from "../../../lemon/components/HelpCallout";
import { VectorSettings } from "../../../lemon/components/VectorSettings";
import { RetrievalMode } from "../../api";
import styles from "../../../lemon/components/Settings/Settings.module.css";
...
export const Settings = ({
    ...
    const chatReasoningOptions: IDropdownOption[] = reasoningEffortOptions.map(option => ({
        key: option,
        text: t(`labels.reasoningEffortOptions.${option}`)
    }));
        ...
        <div className={className}>
            ...
            {availableChatModels.length > 0 && (
                <Dropdown
                    id={chatModelFieldId}
                    className={styles.settingsSeparator}
                    selectedKey={availableChatModels.includes(chatModel) ? chatModel : undefined}
                    label={t("labels.chatModel")}
                    onChange={(_ev?: React.FormEvent<HTMLElement | HTMLInputElement>, option?: IDropdownOption) =>
                        onChange("chatModel", option?.key?.toString() ?? chatModel)
                    }
                    aria-labelledby={chatModelId}
                    options={availableChatModels.map(model => ({ key: model, text: model }))}
                    onRenderLabel={props => renderLabel(props, chatModelId, chatModelFieldId, t("helpTexts.chatModel"))}
                />
            )}
            {showReasoningEffortOption && (
                <Dropdown
            ...
            {showVectorOption && !useAgenticKnowledgeBase && (
                <>
                    <VectorSettings
                        ...
                        updateRetrievalMode={val => onChange("retrievalMode", val as unknown as RetrievalMode)}
