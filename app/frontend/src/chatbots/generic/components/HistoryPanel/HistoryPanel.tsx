...
import { HistoryData, HistoryItem } from "../HistoryItem/HistoryItem";
import { Answers, HistoryProviderOptions, HistoryMetaData } from "../HistoryProviders/IProvider";
...
// Browser-only chat-history drawer for dynamic bots. Unlike the built-in bots' panel there is no MSAL /
// Cosmos path — history lives entirely in IndexedDB, so no id token is threaded through.
export const HistoryPanel = ({
    ...
    notify: boolean;
    onClose: () => void;
    onChatSelected: (id: string, answers: Answers) => void;
}) => {
    ...
    const loadMoreHistory = async () => {
        setIsLoading(true);
        const loadedItems = await historyManager.getNextItems(HISTORY_COUNT_PER_LOAD);
