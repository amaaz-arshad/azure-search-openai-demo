#!/usr/bin/env python3
"""End-to-end refresh of the snap.de dataset in Azure AI Search (single manual command).

Runs the whole pipeline in order:

  1. **Change check** — compares a cheap WP REST watermark (latest ``modified`` +
     record count for pages/posts) against ``data/snap.state.json`` from the last
     successful refresh. If nothing changed it exits without touching anything,
     unless ``--force`` is given.
  2. **Scrape** — runs ``scripts/scrape_snap.py`` to (re)write ``data/snap.json`` as
     markdown. Aborts (without deleting anything) if the scrape fails or yields no docs.
...
# Reuse the scraper's WP-API watermark helper so all snap.de REST knowledge lives in one module.
sys.path.insert(0, str(REPO_ROOT / "scripts"))
from scrape_snap import DEFAULT_BASE_URL, fetch_remote_state  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape snap.de and refresh the 'snap' category in Azure AI Search.")
    ...
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="Only report whether snap.de changed since the last refresh; do not scrape or index",
    )
    args = parser.parse_args()
    base_url = args.base_url.rstrip("/")

    # Step 1: change detection.
    log(f"Checking snap.de for changes ({base_url}) ...")
    ...
    run_step("Scrape snap.de -> data/snap.json", [sys.executable, str(SCRAPER), "--base-url", base_url], cwd=REPO_ROOT)
    ...
    log("DONE: snap.de re-scraped and re-indexed under category 'snap'.")
    return 0
