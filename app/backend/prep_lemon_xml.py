...
async def upload_dataset_blob(blob_manager: BlobManager, dataset_path: Path, blob_prefix: str) -> str:
    ...
    logger.info("Uploading raw lemon knowledge dataset blob '%s'", blob_name)
...
async def remove_existing_lemon_documents(
...
async def run(args: argparse.Namespace) -> None:
    from prepdocslib.lemonxml import prepare_lemon_xml_sections
    from prepdocslib.searchmanager import SearchManager
    ...
    try:
        ...
        root = ET.fromstring(dataset_path.read_bytes())

        with dataset_path.open("rb") as dataset_file:
            file = File(dataset_file, acls=global_acls)
            sections = prepare_lemon_xml_sections(root, file=file, category=args.category)

        if not args.preserveexisting:
            await remove_existing_lemon_documents(
                search_manager,
        ...
        logger.info(
            "Indexed %d lemon knowledge search documents and uploaded raw dataset blob to %s",
            len(sections),
            dataset_storage_url,
        )
    finally:
...
if __name__ == "__main__":  # pragma: no cover
    default_dataset_path = Path(__file__).resolve().parents[2] / "data" / "lemon_demo_knowledge.xml"
    parser = argparse.ArgumentParser(
        description="Ingest the lemon knowledge XML file into Azure AI Search with lemon-specific mapping."
    )
    parser.add_argument(
        "file",
        nargs="?",
        default=str(default_dataset_path),
        help="Path to the lemon knowledge XML file to ingest",
    )
    ...
    parser.add_argument(
        ...
        help="Blob path prefix under the content container for the uploaded raw lemon knowledge dataset file",
    )
    parser.add_argument(
        "--preserveexisting",
        action="store_true",
        help="Do not purge existing lemon knowledge documents for this file/category before indexing",
    )
    parser.add_argument(
        "--disablebatchvectors",
        action="store_true",
