...
class CategoryUploadStrategy:
    ...
    async def add_file(self, file: File, category: str, upload_id: Optional[str] = None) -> CategoryUploadEntry:
        ...
        try:
            ...
            return self.manifest_to_entry(manifest)
    ...
    async def iter_manifest_blob_names(self, category: Optional[str] = None) -> list[str]:
        prefix = f"{self.manifest_prefix(category)}/" if category is not None else ""
        blob_names = await self.blob_manager.list_blob_names(prefix)
        return [blob_name for blob_name in blob_names if MANAGED_UPLOAD_MANIFEST_PATTERN.match(blob_name)]

    async def list_entries(self, category: Optional[str] = None) -> list[CategoryUploadEntry]:
        entries: list[CategoryUploadEntry] = []
        for manifest_blob_name in await self.iter_manifest_blob_names(category=category):
            manifest_category = self.manifest_category_from_blob_name(manifest_blob_name)
            if manifest_category is None:
                continue
            token = manifest_blob_name.rsplit("/", 1)[-1][:-5]
            try:
                filename = self.decode_token(token)
            except Exception:
                logger.warning("Skipping unreadable managed upload manifest blob %s", manifest_blob_name)
                continue
            manifest = await self.get_manifest(manifest_category, filename)
            if manifest is None:
                continue
            entries.append(self.manifest_to_entry(manifest))
        return sorted(entries, key=lambda entry: (entry.category, entry.filename.lower()))

    async def list_categories(self) -> list[str]:
        return sorted({entry.category for entry in await self.list_entries()})
