...
* app: Contains the main application code, including frontend and backend.
  * app/backend: Contains the Python backend code, written with Quart framework.
    ...
    * app/backend/prepdocslib: Contains the document ingestion library used by both local and cloud ingestion
      ...
      * app/backend/prepdocslib/filestrategy.py: Strategy helpers for local ingestion, authenticated user uploads in ADLS, and shared chatbot uploads in blob storage. `ChatbotUploadStrategy` powers the demo-style public upload flows using local parsers only, indexes uploads into the chatbot's normal category, rejects filename collisions with built-in chatbot content, stores physical upload blobs under versioned `chatbot-uploads/<chatbot_name>/files/...` paths with manifest files in `.manifests/`, supports cooperative cancellation through blob-backed cancel markers in `.cancel/`, and now also supports per-chatbot upload rules such as allowed file extensions and maximum total PDF pages across uploaded files.
  ...
  * app/frontend: Contains the React frontend code, built with TypeScript, built with vite.
    ...
    * app/frontend/src/chatbots/<chatbot_name>: Chatbot-specific frontend implementation (pages, components, layout wrapper, i18n, locales, assets, and chatbot wiring).
      ...
      * app/frontend/src/chatbots/public-test: Chatbot implementation cloned from `demo`, including the same demo-style basic-auth gate and shared upload-manager modal. It currently accepts only PDF uploads and enforces a shared limit of 30 total PDF pages across all uploaded files.
...
* infra: Contains the Bicep templates for provisioning Azure resources. `infra/main.bicep` grants the backend managed identity `Storage Blob Data Contributor` on the main storage account and `Search Index Data Contributor` on Azure AI Search because shared chatbot uploads (currently the demo and public-test upload flows) write to blob storage and the search index even when `USE_USER_UPLOAD=false`.
