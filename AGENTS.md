...
* Chatbot basic auth for `agindo`, `demo`, `fbn`, `fhg`, `internal`, `knoll`, `moodle`, `rak`, `sartorius`, `steuertipps`, and `vjoonk4` is enforced server-side through `/chatbot-auth/<chatbot_name>/*` HttpOnly session cookies. Keep frontend login wrappers as API clients only; do not reintroduce hardcoded browser-side credential checks.
